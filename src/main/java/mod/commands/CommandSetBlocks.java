...
public class CommandSetBlocks extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 7) {
			...
			if (j > 100000000) {
				throw new CommandException("commands.setblocks.tooManyBlocks",
						new Object[] { Integer.valueOf(j), Integer.valueOf(100000000) });
				...
				if (args.length >= 10 && block.hasTileEntity(state)) {
					...
					try {
						...
						throw new CommandException("commands.setblocks.tagError",
				...
				if (j <= 0) {
					...
					notifyCommandListener(sender, this, "commands.setblocks.success",
	...
	public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args,
		...
		return args.length > 0
				&& args.length <= 3
						? getTabCompletionCoordinate(args, 0, pos)
						: (args.length > 3 && args.length <= 6 ? getTabCompletionCoordinate(args, 3, pos)
								: (args.length == 7 ? getListOfStringsMatchingLastWord(args, Block.REGISTRY.getKeys())
										: (args.length == 9
